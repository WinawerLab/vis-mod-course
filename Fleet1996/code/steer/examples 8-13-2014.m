...
for lev = 1:numLevels
  for orientation = 1:numOrientations
      band = accessSteerBand(pyr,pind,numOrientations,lev,orientation);
      evenband = real(band);
      oddband = imag(band);
      displayImage(band);
      xlswrite(['~/Dropbox/Teaching/Tutorials Excel/Filters/RF-even-',...
          int2str(lev),'-',int2str(orientation)],evenband);
      xlswrite(['~/Dropbox/Teaching/Tutorials Excel/Filters/RF-odd-',...
          int2str(lev),'-',int2str(orientation)],oddband);
      pause(.5);
  end
end
